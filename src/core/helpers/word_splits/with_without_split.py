...
def get_with_letter_dataset(letter: str, name_letter: str, word_df: pd.DataFrame,
                            folder_path: str) -> pd.DataFrame:
...
def get_without_letter_dataset(letter: str, name_letter: str, word_df: pd.DataFrame, 
                               folder_path: str,num_of_samples: int) -> pd.DataFrame:
    df_without = word_df[
        ~word_df["label"].str.contains(letter, regex=False)].copy()
    without_letter_dataset = df_without.sample(n=min(num_of_samples, len(df_without)), random_state=42).reset_index(drop=True)
...
def get_with_without_split(letters: set, output_path: str, word_df: pd.DataFrame):
    ...
    for letter in letters:
        ...
        if letter in SYMBOL_NAMES:
            name_letter = SYMBOL_NAMES[letter]
        try:        
            folder_path = os.path.join(output_root, f"{name_letter}_letter")
            with_letter_dataset = get_with_letter_dataset(letter, name_letter, word_df, folder_path)
            without_letter_dataset = get_without_letter_dataset(letter, name_letter, word_df, folder_path, len(with_letter_dataset))
            data = {
                "letter": letter,
                "with": with_letter_dataset[["file_name", "label"]].to_dict(orient="records"),
                "without": without_letter_dataset[["file_name", "label"]].to_dict(orient="records"),
            }
