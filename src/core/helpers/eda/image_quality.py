...
def is_noisy(name: str, img_dir: str, NOISE_STD_MEAN_THRESHOLD: int) -> bool:
    ...
    try:
        ...
        return (np.std(img) / np.mean(img)) > NOISE_STD_MEAN_THRESHOLD if np.mean(img) > 0 else False
    except:
        return False
...
def check_image_quality(df: pd.DataFrame, img_dir: str, BLANK_STD_THRESHOLD: int, 
                    SHARP_LAPLACIAN_THRESHOLD: int, NOISE_STD_MEAN_THRESHOLD: int, 
                    CONTRAST_DIFF_THRESHOLD: int) -> None:
    """Flag images that are blank, blurry, noisy or low-contrast."""
    df["is_blank"] = df["Filenames"].apply(lambda f: is_blank(f, img_dir, BLANK_STD_THRESHOLD))
    df["is_sharp"] = df["Filenames"].apply(lambda f: is_sharp(f, img_dir, SHARP_LAPLACIAN_THRESHOLD))
    df["is_noisy"] = df["Filenames"].apply(lambda f: is_noisy(f, img_dir, NOISE_STD_MEAN_THRESHOLD))
    df["is_low_contrast"] = df["Filenames"].apply(lambda f: is_low_contrast(f, img_dir, CONTRAST_DIFF_THRESHOLD))
