...
def filter_contours(
    dilated: np.ndarray, img_shape: tuple, min_area_ratio: float = 0.001
):
    """
    ...
    big_contours, _ = cv2.findContours(
...
def build_word_dataset(word_pairs, base_name):
    ...
    data = {"image": list(images), "label": list(labels)}
...
def visualize_results(
    ...
    """
    Visualizes words (green boxes), noise (white fill),
    optional token labels (red), and plots the cleaned-mask vertical projection.
    """
    ...
    for cnt in small_contours:
        x, y, w, h = cv2.boundingRect(cnt)
        cv2.rectangle(
            vis, (x, y), (x + w, y + h), (255, 255, 255), thickness=cv2.FILLED
        )
...
def extract_words_with_labels(img, mask, big_contours, tokens):
    """
    Returns a list of (word_img, label_text); requires contours to be pre-sorted.
    """
    word_pairs = []
...
def build_full_word_dataframe(train_set, output_dir):
    """
    Build a word-level dataset from a line-level training set and save it as CSV.
    """
    word_dfs = []
    ...
    for row_idx in range(len(train_set)):
        ...
        if len(tokens) == len(big_ctrs_sorted):
            ...
            print(
                f"row {row_idx}: contours={len(big_ctrs_sorted)} tokens={len(tokens)}"
            )
